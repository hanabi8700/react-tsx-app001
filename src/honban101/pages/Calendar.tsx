...
// １週間のバックグラウンド関数
const weekDayBG = (ctDate: string, weeksNum: number, dataset: string[]) => {
...
// 1週間のフォアグラウンド関数
const weekDayFG = (ctDate: string, weeksNum: number, dataset: string[]) => {
  const weekdayArray = calc.getWeekDay7(ctDate, weeksNum);
  //today.format="2024/06/05"
  // const newDataset = dataset.findIndex((data) => data.name === 'bbb');
...
export const Calendar = () => {
  ...
  // index 0:きょう  1~n:祝日
  const holidayList = [calc.getDateWithString(calendarDates.today as Date)];
